...
namespace Enterprises.Admin
{
    public partial class frmCategory : Form
    {
        ...
        {
            if (MessageBox.Show("are you sure want to delete ? [" + CATEGORY.Text.ToLower() + "]", "Category Delete", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                Insert_Update_Delete_Category("DELETE");
            }
        }

        private void SAVE_Click(object sender, EventArgs e)
        {
            Insert_Update_Delete_Category("INSERT");
        }

        private void EDIT_Click(object sender, EventArgs e)
        {
            Insert_Update_Delete_Category("UPDATE");
        }
        ...
        public void Insert_Update_Delete_Category(string BUTTON_TYPE)
        {
            try
            {
                if (CATEGORY.Text == "")
                {
                    MessageBox.Show("Enter Category Name", _variables.SHOP_NAME, MessageBoxButtons.OK, MessageBoxIcon.Information);
                    CATEGORY.Focus();
                }
                else
                {
        ...
        private void CATEGORY_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                dal.ClearParameters();
                dal.AddParameter("@TYPE", "MASTER_TABLE", "IN");
                dal.AddParameter("@SUB_TYPE", "SEARCH_BY_NAME", "IN");
                dal.AddParameter("@PAGE_NAME", PAGE_NAME.ToString(), "IN");
                dal.AddParameter("@NAME_1", CATEGORY.Text.Trim(), "IN");
                dal.AddParameter("@BRANCH_ID", _variables.BRANCH_ID, "IN");
                dt = dal.GetTable("SP_MASTER_TABLE", ref Message);
                grdData.DataSource = dt;
                grdData.Columns["ID"].Visible = false;
        ...
        private void grdData_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                CATEGORY.Text = grdData.CurrentRow.Cells["CATEGORY"].Value.ToString().Trim();

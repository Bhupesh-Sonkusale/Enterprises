    ...
    public partial class Expense : Form
    {
        ...
        public void Insert_Update_Delete_Expense(string BUTTON_TYPE)
        {
            try
            {
                if (CATEGORY.Text == "")
                {
                    MessageBox.Show("Enter Expense Name", _variables.SHOP_NAME, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    CATEGORY.Focus();
                }
                else
                {
        ...
        private void grdData_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                CATEGORY.Text = grdData.CurrentRow.Cells["EXPENSE"].Value.ToString().Trim();

...
{
    public partial class Balance_Paid : Form
    {
        DataTable dt;
        string Message = "";
        int _result = 0;
        DAL dal = new DAL();
        public string USER_ID; string CUSTOMER_ID = "";
        Shop_Detail _variables = new Shop_Detail();
        Validation _validation = new Validation(); public bool FINANCIAL_STATUS;
        ...
        private void Balance_Paid_Load(object sender, EventArgs e)
        {
            Show_All();
        }
        ...
        public void Show_All()
        {
            try
            {
                ...
                dal.AddParameter("@SUB_TYPE", "BALANCE_LIST", "IN");
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    grdData.Columns["CUSTOMER_ID"].Visible = false;
                    grdData.Columns[0].Width = 150;
                    grdData.Columns[2].Width = 550;
        ...
        private void grdData_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                CUSTOMER_ID = grdData.CurrentRow.Cells[1].Value.ToString();
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    decimal TOTAL = CASH_ + CARD_ + ONLINE_;
                    string IN_WORDS = _convertnumbertowords.ConvertNumbertoWords(Convert.ToInt32(TOTAL));
                    ...
                    dal.AddParameter("@PARTY_NAME", NAME_MOBILE.Text, "IN");
                    dal.AddParameter("@BRANCH_ID", _variables.BRANCH_ID, "IN");
                    ...
                    dal.AddParameter("@CUSTOMER_ID", CUSTOMER_ID.ToString(), "IN");
                    ...
                    if (_result > 0)
                    {
                        var RESULT = dal.ReturnParameter.Split('~');
                        MessageBox.Show(RESULT[0], _variables.SHOP_NAME, MessageBoxButtons.OK, MessageBoxIcon.Information);
                        Show_All();
                        btnDetail_Click(sender, e);

                        //Bill_Rdlcs.CustomerReceipt obj = new Bill_Rdlcs.CustomerReceipt();
                        //obj.RECEIPT_ID = RESULT[1];
                        //obj.Show();
                    }
                }
            }
        ...
        private void SEARCH_BY_NAME_MOBILE_TextChanged(object sender, EventArgs e)
        {
            try
            {
                dal.ClearParameters();
                dal.AddParameter("@TYPE", "SALE", "IN");
                dal.AddParameter("@SUB_TYPE", "BALANCE_LIST_BY_FILTER", "IN");
                dal.AddParameter("@BRANCH_ID", _variables.BRANCH_ID, "IN");

    ...
    public partial class Purchase : Form
    {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dal.AddParameter("@SUB_TYPE", "PURCHASE_DATEWISE", "IN");
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < grdData.Rows.Count; i++)
                    {
                        ...
                        PAID_ += Convert.ToDecimal(grdData.Rows[i].Cells[5].Value);
                        BALANCE_ += Convert.ToDecimal(grdData.Rows[i].Cells[6].Value);
                    }

                    DataRow drToAdd = dt.NewRow();
                    drToAdd[3] = "Total";
                    drToAdd[4] = TOTAL_.ToString();
                    drToAdd[5] = PAID_.ToString();
                    drToAdd[6] = BALANCE_.ToString();

                    dt.Rows.Add(drToAdd);
                    dt.AcceptChanges();
                    grdData.DataSource = dt;
                    grdData.Columns[0].Width = 120;
                    grdData.Columns[1].Width = 120;
                    grdData.Columns[2].Width = 120;
        ...
        private void grdData_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            {
                panelDetail.Show();
                string PURCHASE_ID = grdData.CurrentRow.Cells[0].Value.ToString();
                string BILL_ID = grdData.CurrentRow.Cells[1].Value.ToString();

                dt = new DataTable();
                lblBillID.Text = "Purchase ID : " + PURCHASE_ID;
                lblBillIDS.Text = "Bill ID : " + BILL_ID;

                dal.ClearParameters();
                dal.AddParameter("@TYPE", "PURCHASE_ITEM", "IN");
                dal.AddParameter("@SUB_TYPE", "DETAIL_WITH_ID", "IN");
                dal.AddParameter("@BRANCH_ID", _variables.BRANCH_ID, "IN");
                ...
                grdBillDetail.Columns[0].Width = 400;
                grdBillDetail.Columns[1].Width = 150;
                grdBillDetail.Columns[2].Width = 150;
                grdBillDetail.Columns[3].Width = 150;
                grdBillDetail.Columns[4].Width = 150;
            }
            catch (Exception ex) { }
        }

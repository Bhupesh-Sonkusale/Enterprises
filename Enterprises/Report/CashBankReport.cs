...
{
    public partial class CashBankReport : Form
    {
        DataTable dt;
        string Message = "";
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    Show_Sale_Receipt_Details();
                }
            }
            catch (Exception ex) { }
        }

        public void Show_Sale_Details()
        {
            dt = new DataTable();
            dal.ClearParameters();
            dal.AddParameter("@TYPE", "REPORT", "IN");
            dal.AddParameter("@SUB_TYPE", "SALE_CONCELATION_REPORT", "IN");
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                decimal TOTAL = 0, CASH = 0, CARD = 0, ONLINE = 0;
                try
                {
                    for (int i = 0; i < grdData.Rows.Count; i++)
                    {
                        ...
                        CARD += Convert.ToDecimal(grdData.Rows[i].Cells[4].Value);
                        ONLINE += Convert.ToDecimal(grdData.Rows[i].Cells[5].Value);
                    }
                }
                catch (Exception ex) { }
                ...
                drToAdd[4] = CARD.ToString();
                drToAdd[5] = ONLINE.ToString();

                grdData.Columns[0].Width = 100;
                grdData.Columns[1].Width = 400;
                grdData.Columns[2].Width = 100;
        ...
        public void Show_Sale_Receipt_Details()
        {
            dt = new DataTable();
            dal.ClearParameters();
            dal.AddParameter("@TYPE", "REPORT", "IN");

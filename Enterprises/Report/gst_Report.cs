...
namespace Enterprises.Report
{
    public partial class gst_Report : Form
    {
        ...
        string FROM_DATE_;
        string TO_DATE_; public string BILL_LABLE;
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dal.AddParameter("@SUB_TYPE", "SALE_GST_DATEWISE", "IN");
                ...
                dal.AddParameter("@BILL_LABLE", BILL_LABLE, "IN");
                dal.AddParameter("@FINANCIAL_STATUS", FINANCIAL_STATUS, "IN");
                dt = dal.GetTable("SP_SALE", ref Message);
                if (dt.Rows.Count > 0)
                {
                    grdData.DataSource = dt;
                    decimal TOTAL_ = 0, CGST_ = 0, SGST_ = 0,TAXABLE_ = 0;
                    try
                    {

                        for (int i = 0; i < grdData.Rows.Count; i++)
                        {
                            TAXABLE_ += Convert.ToDecimal(grdData.Rows[i].Cells[2].Value);
                            CGST_ += Convert.ToDecimal(grdData.Rows[i].Cells[3].Value);
                            SGST_ += Convert.ToDecimal(grdData.Rows[i].Cells[4].Value);
                            TOTAL_ += Convert.ToDecimal(grdData.Rows[i].Cells[5].Value);
                        }
                    }
                    catch (Exception ex) { }
                    ...
                    drToAdd[1] = "Total";
                    drToAdd[2] = TAXABLE_.ToString();
                    drToAdd[3] = CGST_.ToString();
                    drToAdd[4] = SGST_.ToString();
                    drToAdd[5] = TOTAL_.ToString();
        ...
        private void btnSearchByGST_Click(object sender, EventArgs e)
        {
            grdData.DataSource = null;
            try
            {
                ...
                dal.ClearParameters();
                dal.AddParameter("@TYPE", "REPORT", "IN");
                dal.AddParameter("@SUB_TYPE", "SALE_GST_DATEWISE_BY_GST", "IN");
                dal.AddParameter("@BRANCH_ID", _variables.BRANCH_ID, "IN");
                dal.AddParameter("@FROM_DATE", FROM_DATE_, "IN");
                dal.AddParameter("@TO_DATE", TO_DATE_, "IN");
                dal.AddParameter("@BILL_LABLE", BILL_LABLE, "IN");

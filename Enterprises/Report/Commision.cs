...
namespace Enterprises.Report
{
    public partial class Commision : Form
    {
        ...
        private void Commision_Load(object sender, EventArgs e)
        {
            Bind_AdminEmployee();
            ORDER_BY.SelectedIndex = 0;
        }

        #region -- Code For Commision
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    dal.AddParameter("@SUB_TYPE", "COMMISION_REPORT", "IN");
                    dal.AddParameter("@BRANCH_ID", _variables.BRANCH_ID, "IN");
        ...
        public void GridDesign()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                decimal TOTAL_ = 0, COMMISION_AMOUNT_ = 0,_MUDY_AMOUND = 0;
                try
                {

                    for (int i = 0; i < grdData.Rows.Count; i++)
                    {
                        TOTAL_ += Convert.ToDecimal(grdData.Rows[i].Cells[3].Value.ToString().Trim());
                        _MUDY_AMOUND += Convert.ToDecimal(grdData.Rows[i].Cells[5].Value.ToString().Trim());
                        COMMISION_AMOUNT_ += Convert.ToDecimal(grdData.Rows[i].Cells[6].Value.ToString().Trim());
                    }
                }
                catch (Exception ex) { }
                ...
                drToAdd[5] = " " + _MUDY_AMOUND.ToString();
                drToAdd[6] = " " + COMMISION_AMOUNT_.ToString();
            ...
            grdData.Columns[6].Width = 170;
        }

        #endregion -- Code For Commision
    }
}
